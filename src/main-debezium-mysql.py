...
def load_config_from_args() -> JobConfig:
    parser = argparse.ArgumentParser(
        description="Debezium CDC to MySQL Flink Job",
    ...
    parser.add_argument("--mysql-table", default="cdc_sync_data", help="MySQL table")
...
def run_job(config: JobConfig):
    ...
    create_source_sql = f"""
        CREATE TABLE debezium_source (
            ...
            'properties.group.id' = 'flink-debezium-mysql-consumer',
    ...
    try:
        ...
        logger.info("Flink CDC job started, syncing data to MySQL...")
        result.wait()
